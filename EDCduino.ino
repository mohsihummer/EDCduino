...
This software should be considered pre-alpha in its current implementation
Uses Tunerstudio for comms and calibration
Features - Crankangle interception, rail pressure interception, 2xPWM channels i.e Boost and VVT, Thermo-fan control
...
#include "corrections.h"// no corrections implemented
#include "decoders.h"// returns rpm. decoding, in this code requires changing value in this file before uploading.
...
void loop()
{
  ...
  if  ((currentStatus.RPM > 1000) && (currentStatus.RPM < 10000)) //Check how long ago the last tooth was seen compared to now. If it was more than half a second ago then the engine is probably stopped. toothLastToothTime can be greater than currentLoopTime if a pulse occurs between getting the lastest time and doing the comparison
  {
    ...
    //target_degs =(uint16_t) currentStatus.advance - 5;
        if(syncstat>=3){
          target_degs = (uint16_t) currentStatus.advance - 5; 
          if(target_degs>5)//limiting to max values in case of improper inputs from table
          {target_degs=5;}
          if (target_degs < -5){target_degs= -5;}}
...
ISR(TIMER5_CAPT_vect)//rising
{
  crinteeth++;
   delta_t1 = (uint16_t)ICR5;  t1 = TCNT5;
  ...
  { 
    ...
    { croteeth++; //PORTD ^= (_BV(PD2));      PORTD ^= (_BV(PD2)); iofH = 0;
      if ((croteeth != cromiss1) && (croteeth != cromiss2)) {
        if ((croteeth != cromiss3) && (croteeth != cromiss4)) // check for missing teeth on crANKO
        {          PORTD |= _BV(PD3);        }
      }
      if ((crinteeth != 13) && (crinteeth != 71)) {        delta_t = delta_t1 - delta_t2;      }
      per24thteethH = (((uint32_t)delta_t * (uint32_t)0xAAAB) >> 20) ; //fast div by 24
      c = (ICR5 + (20 * (per24thteethH))-13) ;
      if (iofH <= 0) {
        OCR3A = (uint16_t) c;
        iofH = 1;
      }
    ...
    PORTD |= _BV(PD3); 
    if(crinteeth==33)//crank low in case of no sync
    {PORTD &= ~_BV(PD2);}
    }
...
{
//if((cur_degsL==0) && (aL==0)){PORTD &= ~_BV(PD3); syncstat = 2;}
 ...
 if (syncstat >= 3)
  {
    ...
    { OCR3B= ICR4-10;
      ...
      if (misteeth1 == 1) {
        ...
        OCR3B = (uint16_t)b;
        //TIFR3 |= _BV(OCF3B);        TIMSK3 |= _BV(OCIE3B);
        misteeth1++;
      }
    }
    else if ((aL == 0) && (aL1  == 1)) {
      //PORTD ^= (_BV(PD2));      PORTD ^= (_BV(PD2));iofL=0;
      if ((croteeth != cromiss1) && (croteeth != cromiss2)) {
        if ((croteeth != cromiss3) && (croteeth != cromiss4)) // check for missing teeth on crANKO
        {          PORTD &= ~_BV(PD3);        }
      }
      if ((crinteeth != 13) && (crinteeth != 71)) {        deltaL_t = deltaL_t1 - deltaL_t2;      }
      per24thteethL = (((uint32_t)deltaL_t * (uint32_t)0xAAAB) >> 20) ; //fast div by 24
      b = (ICR4 + (20 * (per24thteethL))-5) ;aL1=0;
       if (iofL <= 0)
      {        OCR3B = (uint16_t) b;       /* TIFR3 |= _BV(OCF3B);  TIMSK3 |= _BV(OCIE3B); */  iofL = 1;      }
      else {        OCR3B1 = (uint16_t) b;        iofL = 2;      }
    }
    else if (aL != 0 )
  ...
  {if (cur_degsL== -1){OCR3B= ICR4-10;OCR3A = ICR5-10;}
    cur_degsL++;
    aL = (cur_degsL) % 6;
  }
  ...
  //if(TCNT3<200){ PORTD ^= (_BV(PD2));      PORTD ^= (_BV(PD2));}
}
...
{     croteeth++;
  ...
  //if((misteeth==3)&&(aL==0)){TIMSK3 &= _BV(OCIE3A);}
...
{  PORTD &= ~_BV(PD3);
  ...
  //if ((misteeth1 == 3) && (aL == 0) && (misteeth == 3))
  //{    TIMSK3 &= _BV(OCIE3B);    TIMSK3 &= _BV(OCIE3A);  }

  if ((misteeth1 == 1) || (misteeth1 == 2)) //crin missing teeth no advance interrupt
  { //PORTD ^= _BV(PD2); PORTD ^= _BV(PD2);
    e = OCR3B + deltaL_t ;    OCR3B = (uint16_t) e ;    /*TIMSK3 |= _BV(OCIE3B);*/
    misteeth1++;
  }
  if (iofL <= 1) {
    iofL = 0;
...
ISR(INT0_vect)
{
  ...
  if(syncstat==3){syncstat++;TIFR3 |= _BV(OCF3A);TIFR3 |= _BV(OCF3B);TIMSK3 |= _BV(OCIE3A);TIMSK3 |= _BV(OCIE3B); }
  if((cur_degsL==0)&&(target_degs==0)){croteeth=1;}
}
