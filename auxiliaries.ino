...
void initialiseAuxPWM()
{
 ...
 TIMSK1 |= _BV(OCIE1A); //Turn on the A compare unit (ie turn on the interrupt)  
 TIMSK1 |= _BV(OCIE1B);//Turn on the B compare unit (ie turn on the interrupt)
...
else { TIMSK1 &= _BV(OCIE1A); } // Disable timer channel
}
...
void vvtControl()
{
  ...
  else { TIMSK1 &= _BV(OCIE1B); } // Disable timer channel
